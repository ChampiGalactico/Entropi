...
interface SubjectGrade { subject: Subject; grade: number | null; weight: number; componentCount: number }
  ...
  useEffect(() => {
    void listAllSubjects().then(async (subjects) => {
      ...
      setRows(await Promise.all(gradable.map(async (subject) => {
        ...
        return { subject, grade: summary.subjectGrade, weight: summary.configuredWeight, componentCount: components.length };
      })));
    });
  }, []);
