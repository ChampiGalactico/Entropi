...
export function WeekView({ weekStart, preferences }: { weekStart: Date; preferences: CalendarPreferences }) {
  ...
  const showNowLine = nowMinutes >= startMinute && nowMinutes <= endMinute;
  ...
  return <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-[1.75rem] border border-border bg-elevated shadow-card backdrop-blur-2xl">
    ...
    <div className="mr-[11px] grid grid-cols-[58px_repeat(7,minmax(0,1fr))] border-b border-border bg-surface/50">
      <div className="flex items-center justify-center px-1 py-2 text-center text-[9px] uppercase text-text-muted">{t("calendar.allDay")}</div>{days.map((day) => { const iso = toIsoDate(day); const allDay = items.filter((item) => item.date === iso && item.startTime === null); return <div key={iso} className="min-h-12 min-w-0 space-y-1 border-l border-border p-1.5">{allDay.map((item) => <div key={item.id} role="button" tabIndex={0} onClick={() => openEntityDetail(item.kind, entityIdFromCalendarItem(item))} title={item.title} className={`cursor-pointer truncate rounded-lg px-2 py-1 text-[10px] font-medium transition-opacity hover:opacity-80 ${item.muted ? "opacity-50" : ""}`} style={{ color: item.color, background: `color-mix(in srgb, ${item.color} 14%, transparent)` }}>{item.title}</div>)}</div>; })}
    </div>
    <div className="min-h-0 flex-1 overflow-y-auto" style={{ scrollbarGutter: "stable" }}>
      <div className="grid grid-cols-[58px_repeat(7,minmax(0,1fr))]" style={{ height: gridHeight + TOP_GUTTER + BOTTOM_GUTTER }}>
        ...
        {days.map((day) => { const iso = toIsoDate(day); const timed = items.filter((item) => item.date === iso && item.startTime !== null && timeToMinutes(item.startTime) < endMinute && timeToMinutes(item.endTime ?? item.startTime) >= startMinute); return <div key={iso} className="relative min-w-0 overflow-hidden border-l border-border" style={{ backgroundPositionY: TOP_GUTTER, backgroundImage: `repeating-linear-gradient(to bottom, transparent 0, transparent ${HOUR_HEIGHT - 1}px, var(--border-subtle) ${HOUR_HEIGHT - 1}px, var(--border-subtle) ${HOUR_HEIGHT}px)` }}>
          {showNowLine && iso === today && (
            <div className="pointer-events-none absolute left-0 right-0 z-20 flex items-center" style={{ top: nowTop }}>
              <span className="-ml-[4px] h-[8px] w-[8px] shrink-0 rounded-full bg-danger" />
              <span className="h-px flex-1 bg-danger" />
            </div>
          )}
          {timed.map((item) => { const rawStart = timeToMinutes(item.startTime!); const rawEnd = item.endTime ? timeToMinutes(item.endTime) : rawStart + 35; const start = Math.max(startMinute, rawStart); const end = Math.min(endMinute, rawEnd); const top = TOP_GUTTER + (start - startMinute) / 60 * HOUR_HEIGHT; const height = Math.max(28, (end - start) / 60 * HOUR_HEIGHT); const subtitleLine = [item.subtitle, item.location].filter(Boolean).join(" · "); return <article key={item.id} role="button" tabIndex={0} onClick={() => openEntityDetail(item.kind, entityIdFromCalendarItem(item))} className={`absolute left-1 right-1 z-10 cursor-pointer overflow-hidden rounded-xl border p-2 shadow-sm transition-all hover:z-20 hover:scale-[1.02] hover:shadow-card ${item.muted ? "opacity-50" : ""}`} style={{ top, height, borderColor: `color-mix(in srgb, ${item.color} 42%, transparent)`, background: `color-mix(in srgb, ${item.color} 16%, var(--bg-elevated))` }}><p className="truncate text-[10px] font-semibold" style={{ color: item.color }}>{formatCalendarTime(item.startTime!, preferences.timeFormat)} · {t(`calendar.kinds.${item.kind}`)}</p><h4 className="truncate text-xs font-semibold text-text-primary">{item.title}</h4>{height > 44 && subtitleLine && <p className="truncate text-[10px] text-text-muted">{subtitleLine}</p>}</article>; })}</div>; })}
      </div>
    </div>
  </div>;
}
