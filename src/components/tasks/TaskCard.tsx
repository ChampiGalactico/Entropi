...
export function TaskCard({ task, type, subject, onToggle, onEdit, onDelete }: { task: Task; type: TaskType | null; subject: Subject | null; onToggle: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <article className={`group rounded-[1.4rem] border border-border bg-control p-4 transition-all hover:-translate-y-0.5 hover:bg-elevated hover:shadow-card ${completed ? "opacity-65" : ""}`} style={{ borderLeftColor: subject?.color ?? type?.color, borderLeftWidth: 3 }}>
      <div className="flex items-start gap-3">
        <div className="pt-0.5"><Checkbox checked={completed} onChange={onToggle} disabled={task.status === "cancelled"} /></div>
