...
export function CalendarSettingsSection() {
  ...
  return <div className="flex flex-col">
    ...
    <div className="flex items-center gap-3 pt-4"><Button disabled={!valid} onClick={() => void save()}>{t("settings.lookup.save")}</Button>{saved && <span className="text-sm text-success">{t("settings.grading.saved")}</span>}</div>
  </div>;
}
