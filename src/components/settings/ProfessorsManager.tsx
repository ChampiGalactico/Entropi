...
import { Button, EmptyState, IconButton, notify } from "../ui";
...
export function ProfessorsManager() {
  ...
  async function remove(professor: Professor) {
    if (!(await confirmDelete({ itemName: professor.name }))) return;
    ...
    {professors.length === 0 ? <EmptyState title={t("settings.professors.empty")} /> : <div className="overflow-hidden rounded-2xl border border-border">
      <table className="w-full text-sm">
        ...
        <tbody>{professors.map((professor) => <tr key={professor.id} onDoubleClick={() => show(professor)} className="cursor-pointer border-b border-border last:border-0 hover:bg-surface-hover">
          ...
          <td className="px-4 py-3"><div className="flex justify-end gap-1"><IconButton label={t("settings.lookup.edit")} icon={<PenLinear size={16} />} onClick={() => show(professor, true)} /><IconButton label={t("settings.lookup.delete")} icon={<TrashBinTrashLinear size={16} />} onClick={() => void remove(professor)} /></div></td>
        </tr>)}</tbody>
      </table>
    </div>}
